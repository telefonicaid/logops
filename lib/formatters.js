...
function formatJsonTrace(level, context, message, args) {
  ...
  var placeholders = countPlaceholders(message);
  var printableArgs = args.splice(0, placeholders); // XXX splice modifies args
  ...
  if (context) {
    log.op = context.op || null;
    log.corr = context.corr || null;
    log.trans = context.trans || null;
  }
