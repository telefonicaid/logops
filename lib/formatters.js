...
function formatDevTrace(level, context, message, args, err) {
  var argsBC = args.map(function(arg) {
    if (arg instanceof Error) {
      // Node 6 now incorporates the stack trace when making console.log(err)
      // so drop that info that is not relevant in this case and use old simpler node4 one
      return '[' + arg.name + ': ' + arg.message + ']';
    }
    return arg;
  });
  var str,
      mainMessage = util.format.apply(global, [message].concat(argsBC)),
